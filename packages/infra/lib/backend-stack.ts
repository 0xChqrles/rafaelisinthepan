...
export class BackendStack extends Stack {
  constructor(scope: Construct, id: string, props: BackendStackProps = {}) {
    ...
    // Viewer address is generated by CloudFront from the actual connection and reaches the
    // origin outside the cache key. The IAM-only Function URL makes it a trusted HMAC input;
    // a viewer-supplied X-Forwarded-For chain is deliberately ignored by the handler. OAC
    // cannot sign a POST to a Lambda URL without the exact viewer-computed body hash.
    //
    // CloudFront forbids explicitly naming any `x-amz-*` header in an origin-request
    // allow-list, so `x-amz-content-sha256` made the old policy undeployable. AWS's managed
    // policy intended for Lambda URL origins uses the same `allExcept: Host` header mode:
    // it forwards viewer headers (including the payload hash) plus generated CloudFront
    // headers (including CloudFront-Viewer-Address), while omitting the viewer Host so the
    // Function URL receives its own origin hostname. Queries and cookies remain independently
    // narrowed below rather than adopting the managed policy's broad all/all settings.
