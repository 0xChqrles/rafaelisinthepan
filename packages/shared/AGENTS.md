...
```
  shared/                     cross-cutting TS consumed by web + backend (pkg @whippin/shared)
    ...
    src/avatar.ts             the #188 avatar: palettes + the 26-byte grid codec (web encodes/renders, backend validates)
...
- `src/avatar.ts` is the ONE definition of the #188 avatar encoding (palette byte + 100
  cells × 2 bits, base64url, canonical decode) and of `AVATAR_PALETTES` — the palette
  list is APPEND-ONLY, since the stored byte is an index into it. The web encodes what
  the editor drew and renders every stored avatar; the backend decodes to validate and
  moderate — a fork would accept different strings for one drawing (root `AGENTS.md`,
  Player profile).
