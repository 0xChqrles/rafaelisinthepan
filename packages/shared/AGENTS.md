...
```
  shared/                     cross-cutting TS consumed by web + backend (pkg @whippin/shared)
    ...
    src/invite.ts             the share paths + #271's group invite link's three paths (infra routes, backend serves, web builds+parses)
...
- `src/invite.ts` is the ONE spelling of the #271 group invite link's paths — the shared
  `/g/<groupId>`, the SPA landing `/join/g/<groupId>` its preview bounces to, and the
  card at `/og/g/<groupId>.png` — beside the share paths. INFRA routes the first to the
  API origin, the BACKEND answers it and writes the landing into its redirect, and the WEB
  builds the link and parses the landing; a drift is an invite that silently lands nobody
  in a group, and it cannot reproduce locally (no CDN in front of the SPA). The product
  rules are in the root `AGENTS.md` (Groups).
