...
# The pure phrase/puzzle contract lives in phrase_core (stdlib-only, shared with the
# benchmark curriculum builder, #84). Re-exported names keep this module's public
# surface unchanged; _make_hole keeps its historical local name.
from phrase_core import (  # noqa: F401
    TOP_K,
    build_lang_config,
    build_rank_map,
    display_token,
    locate_core,
    make_hole as _make_hole,
    normalize,
    ws,
)
...
# --- Per-language config -------------------------------------------------------
# The regex config (char classes, token/strip/core patterns) is phrase_core's; this
# module only attaches each language's embedding module for vector work.
def _build_config():
    config = build_lang_config()
    config["en"]["module"] = gn
    config["fr"]["module"] = frn
    return config
