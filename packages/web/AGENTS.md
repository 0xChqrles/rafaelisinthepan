...
- **Profile editor (#188; two-colour rework + key-UI removal user-decided
  ...
  stored drawing bumps nothing), the palette row — each swatch is the palette's
  FOREGROUND colour, and picking a colour picks the palette; the drawing survives a
  switch — and SAVE (`.mix-btn`). No brush row (two colours need none) and NO key
