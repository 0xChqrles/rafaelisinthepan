...
// the most, the outermost band throws one — the ladder the hit pays by, told in sparks.
//
...
const LOOT_LAUNCH_MS = 100;
const FLIGHT_MIN_MS = 160;

// One spark per table row the charge reaches, counted from the outermost band: the smallest
// pay is one spark, a top-band pay is the whole table's worth.
export function sparkCount(charge: number): number {
  if (charge <= 0) return 0;
  let n = 0;
  for (const [, pay] of CHARGE_TABLE) if (charge >= pay) n += 1;
  return Math.max(1, n);
}
  ...
  const sparks = useMemo(
    () =>
      Array.from({ length: sparkCount(charge) }, () => ({
        ...
        h: roll(0.7, 1.3).toFixed(3),
        delay: Math.round(roll(0, 70)),
      })),
