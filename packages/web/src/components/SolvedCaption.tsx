...
// a kind tag, "— Author, Work", and the fuller passage. Every field is optional, so the
// citation degrades gracefully (nothing rendered when there is no source). The wrapper
// keeps the input row's reserved height either way, so the sentence never shifts when the
// round ends.
export default function SolvedCaption({ source }: { source?: Source }) {
  ...
  return (
    <div className="solved-caption">
      ...
      {source?.context && <p className="solved-context">{source.context}</p>}
    </div>
  );
}
