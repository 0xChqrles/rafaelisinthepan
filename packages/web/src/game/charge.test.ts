...
describe('chargeForRank — the table', () => {
  it('pays each band its charge, edges inclusive', () => {
    expect(chargeForRank(1)).toBe(50);
    expect(chargeForRank(3)).toBe(50);
    expect(chargeForRank(4)).toBe(34);
    expect(chargeForRank(10)).toBe(34);
    expect(chargeForRank(11)).toBe(25);
    expect(chargeForRank(25)).toBe(25);
    expect(chargeForRank(26)).toBe(18);
    expect(chargeForRank(50)).toBe(18);
    expect(chargeForRank(51)).toBe(12);
    expect(chargeForRank(100)).toBe(12);
    expect(chargeForRank(101)).toBe(6);
    expect(chargeForRank(250)).toBe(6);
  });

  it('"close enough" reaches the initial in a handful of guesses (the 2026-09-15 retune)', () => {
    const guessesToFill = (rank: number) => Math.ceil(CHARGE_TARGET / chargeForRank(rank));
    expect(guessesToFill(3)).toBe(2);
    expect(guessesToFill(10)).toBe(3);
    expect(guessesToFill(25)).toBe(4);
    expect(guessesToFill(50)).toBe(6);
    expect(guessesToFill(100)).toBe(9);
  });
...
describe('replayCharge — the meter as the play log describes it', () => {
  ...
  it('a guess that is NOT a new best still charges the hole', () => {
    ...
    // …and each pays the meter by its own rank: 50 + 18 + 12.
    expect(replayCharge(holes(), RANKS, log)[0]).toEqual({ charge: 80, revealed: false });
  });

  it('caps at the target and reveals the moment it is reached', () => {
    // 18 + 18 + 25 + 25 = 86: not yet.
    const four = ['honnete26', 'honnete50', 'honnete11', 'honnete25'];
    expect(replayCharge(holes(), RANKS, four)[0]).toEqual({ charge: 86, revealed: false });
    // 86 + 12 = 98: still not.
    expect(replayCharge(holes(), RANKS, [...four, 'honnete51'])[0]).toEqual({
      charge: 98,
      revealed: false,
    });
    // 98 + 12 ≥ 100: capped, revealed.
    const full = replayCharge(holes(), RANKS, [...four, 'honnete51', 'honnete100'])[0];
    expect(full).toEqual({ charge: CHARGE_TARGET, revealed: true });
    // Once full, further near guesses change nothing.
    expect(replayCharge(holes(), RANKS, [...four, 'honnete51', 'honnete100', 'honnete1'])[0])
      .toEqual(full);
  });
  ...
  it('one guess charges several holes, each by its own rank in its own map', () => {
    // `bois` is rank 9 for the first secret (+34) and rank 2 for the second (+50).
    expect(replayCharge(holes(), RANKS, ['bois'])).toEqual([
      { charge: 34, revealed: false },
      { charge: 50, revealed: false },
    ]);
  });
  ...
  it('repeated occurrences of one secret share one meter', () => {
    ...
    expect(meters[0]).toEqual({ charge: 52, revealed: false });
  ...
  it('the exact hit is the solve: it pays nothing, and nothing after it touches the hole', () => {
    ...
    expect(replayCharge(holes(), RANKS, log)[0]).toEqual({ charge: 50, revealed: false });
    // The other hole is untouched by that secret's solve and keeps charging.
    expect(replayCharge(holes(), RANKS, [...log, 'foret2'])[1].charge).toBe(50);
  });
