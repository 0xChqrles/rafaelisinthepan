...
// snapshots, the private history and the streak all belong to an ACCOUNT. When a held
// identity is LEFT — a sign-out,
...
// EPOCH and drop an answer that outlives it (`identityEpoch`, read by the round engine and
...
export function installIdentityScope(): () => void {
  return onIdentityChange(({ previous, next, accountChanged, adopted }) => {
    ...
    // state that ASKED for it: the guess sitting in the outbox waiting to be sent. Clearing
    // there orphaned the append (later guesses then never synced).
    //
    ...
    if (previous === null) {
      if (next !== null) {
        ...
        // never clear: the outbox still holds what THIS device played, and it is owed to the
        // adopted account.
        if (adopted) {
          rearmRoundSync();
          rearmPlayerHistory();
          ...
          // exists, and the engine mints none of its own (#216 trigger rework): the outbox
          // parked behind the PLAY gate — the pending-bootstrap recovery.
          kickRoundSync();
        }
      }
      return;
