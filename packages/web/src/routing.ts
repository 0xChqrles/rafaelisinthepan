...
// The stamp every entry this app PUSHES carries, and the only thing `goBack` trusts.
const APP_ENTRY = { app: true };

// **ONLY A PUSH MAY STAMP; A REPLACE CARRIES WHATEVER THE ENTRY ALREADY HAD.** The stamp
// answers ONE question — is there a screen of ours BEHIND this entry — and that is a fact
// about the entry's PREDECESSOR, which only a push creates. A replace swaps the current
// entry's contents and changes nothing behind it, so claiming it there would say "ours"
// about somebody else's page: the `/` -> `/<lang>` redirect replaces the entry a pasted URL
// landed on, and `dropLangParam` replaces the one an `?lang=` link landed on.
function writeEntry(url: string, replace: boolean): void {
...
// only case where there is something of ours behind it — hence `writeEntry`'s rule that a
// replace never stamps), and the fallback otherwise — a pasted or bookmarked URL has
// whatever came before it in that tab, and that is not ours to send anyone to.
...
// removing a suggestion the player has just overruled is not a place to go back to — and a
// replace that KEEPS the entry's own stamp, per `writeEntry`. Claiming it broke the exact
// journey this parameter exists for: a pasted `/privacy?lang=en` lands on an entry that is
// nobody's, and stamping it there told `goBack` a screen of ours stood behind it — so the
// notice's back control left the site, or in a fresh tab did nothing at all.
