...
describe('the sentence stage', () => {
  it('opens on its own line and stays silent on a miss and on landed guesses (the tap is taught on the meter stage)', () => {
  ...
  it('nudges toward the hole that has resisted longest, then hints on it', () => {
    ...
    expect(coachLine(b.state())).toEqual({ kind: 'near', hole: expect.objectContaining({ rank: 62 }) });
...
describe('coachCopy', () => {
  ...
  it('prints the board’s words in their in-game dress', () => {
    ...
    expect(coachCopy('en', { kind: 'miss', typed: 'violin' }, stage, true)).toBe(
      '[[m:violin]] has nothing to do with the secret word: a [[m:MISS]].',
    );
    ...
    expect(coachCopy('en', { kind: 'activated', word: 'sea', rank: 1 }, stage, true)).toBe(
      'The meter is full! 10 words close to the secret are masked in its tries. Tap [[w:sea^1]], pick a masked word, then press enter to reveal it — it costs a try.',
    );
    expect(coachCopy('fr', { kind: 'activated', word: 'mer', rank: 1 }, stage, false)).toBe(
      'Jauge pleine ! 10 mots proches du secret sont masqués dans ses essais. Clique sur [[w:mer^1]], choisis un mot masqué, puis valide pour le révéler, contre un essai.',
    );
