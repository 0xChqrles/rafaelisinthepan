...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    if args.runs is None:
        # `--in-place` always writes the canonical DEFAULT_RUNS record, so an omitted
        # --runs defaults to it for every model — including Kimi, whose ordinary default
        # is DEFAULT_KIMI_RUNS — so Kimi `--in-place` needs no explicit `--runs 5`.
        if args.in_place:
