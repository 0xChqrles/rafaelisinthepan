...
_V14_JUDGMENT = (
    ...
    "direction; repeated non-improvement lowers that support. Similarity among your "
    "own guesses is not evidence of progress. Decide for yourself how to adapt."
)
...
def _v14_opening_message(referee: PuzzleReferee) -> str:
    ...
    return "\n".join(
        [
            ...
            "GOAL",
            (
                "Solve every hidden word in as few counted tries as possible. Your "
                "score is the total number of unique valid guesses — lower is better, "
                "so every counted guess is expensive."
            ),
            "",
            "RULES",
            (
            ...
            (
                ...
                "closeness rank: lower is closer, 0 is solved, and MISS is outside the "
                "stored neighborhood."
            ),
            (
            ...
            (
                ...
                "how you will pick each guess, how you will read rank evidence, and "
                "what will make you abandon a direction. It is your plan — no one "
                "holds you to it but yourself."
            ),
            _V14_JUDGMENT,
            "",
            "FEEDBACK FORMAT",
            ...
            *_v14_state_snapshot_lines(referee),
            ...
            _V14_JUDGMENT,
            (
                "First reply: your method, in free text — no guess yet. Every reply "
                "after it must be exactly one word."
            ),
        ]
    )


def _v14_method_noted_message(referee: PuzzleReferee) -> str:
    return "\n".join(
        [
            (
...
def play_puzzle(
    ...
    *,
    ...
    if v14_baseline:
        raw_method = model_reply([message.copy() for message in messages])
        ...
        messages.extend(
            [
                ...
                {
                    "role": "user",
                    "content": _v14_method_noted_message(referee),
                },
            ]
        )
        ...
        messages.append(
            {
                "role": "user",
                "content": (
                    _v14_feedback_message(feedback, referee)
                    if v14_baseline
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    if args.v14_baseline and args.playbook is not None:
