...
//                 { token, leave: <groupId> }   — the caller walks out;
//                 { token, remove: <groupId>, member: <publicId> }
//                                         — the CREATOR shows a member out.
//
...
export async function handleGroups(
  ...
  if (verb === 'leave') {
    await deps.groups.leave(target, publicId);
    return mine();
  }

  // REMOVE: the creator shows a member out. Authorized by the group row, never by the
  // caller's say-so; and never on themselves — that is `leave`, and a creator who leaves
  // simply leaves (the group keeps its members and its link).
...
// each with who is in it (one consistent Query per group, GROUPS_MAX at most) — what the
// board's tabs, the global board's marks and the landing's "already a member" all read.
export async function listGroups(groups: GroupStore, publicId: string): Promise<GroupSummary[]> {
  ...
  const members = await Promise.all(mine.map((group) => groups.members(group.id)));
  return mine.map((group, i) => ({ ...group, members: members[i].map((member) => member.publicId) }));
}
