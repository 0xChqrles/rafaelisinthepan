...
- You have exactly four moves, and each line is told which one to use, and often which KIND of it (the lines are written apart, so the variety is decided for you). No two lines of yours are built the same way: a move is an idea, never a sentence template, and the line does not begin the way the obvious line of that move would.
  ...
  3. THE QUALITY OF A SOMETHING. "la <quality> d'un <noun> <state>" — the quality the score is about (precision, efficiency, patience, flair), the first creature or object anyone would associate with it or with its absence, and ONE adjective for its state (or "de <material>"). The state must be REAL — a condition a body or an object can actually be in: underfed, sick, injured, old, wet, made of a material — never a situation (a job, a strike, a mood): the reader must see it in one glance as a weaker version of the noun. And the pairing must be one nobody has ever had a reason to say: a switched-off appliance or a tired dog is an ordinary thing, not a strange one. No verb, no "qui", no second detail.
