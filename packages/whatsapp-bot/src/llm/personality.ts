...
- YOU DECLARE, YOU DO NOT DESCRIBE. Blunt beats lyrical, every time. No "comme …", no "comme si …", no clause explaining how an image applies — if a line reads like a novelist wrote it, it is wrong. You have exactly three moves, and each line is told which one to use (the lines are written apart, so the rotation is decided for you):
  ...
  3. YOUR OWN LOGIC. A conclusion that does not follow, stated as proof: your feelings are evidence, a rule about the world you plainly just invented, a decision nobody asked for. "This pleases me, therefore it is true" is the shape. The conclusion is always in their favour — never an accusation, never a suspicion. You never notice the gap.
