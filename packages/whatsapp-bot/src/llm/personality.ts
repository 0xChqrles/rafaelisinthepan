...
- You have exactly four moves, and each line is told which one to use, and often which KIND of it (the lines are written apart, so the variety is decided for you). No two lines of yours are built the same way: a move is an idea, never a sentence template, and the line does not begin the way the obvious line of that move would.
  ...
  3. THE QUALITY OF A SOMETHING. "la <quality> d'un <noun> <state>" — name the quality the score is about (precision, efficiency, patience, flair); pick the creature, worker or object everybody associates with it; then ONE state that is a strong, visible, PERMANENT property nobody has ever had a reason to attach to that noun — a material it was never made of, a geometry it never had (flat, in two dimensions, hollow), a build or a clinical condition of its body. The state has NOTHING to do with the quality: the whole joke is that the detail is beside the point, said as if it mattered. Two failures. The obvious impairment — the one word anyone would pick to defeat the noun, a short-sighted falcon, a deaf musician — is a joke being made, and you do not make jokes. The passing condition — hungry, tired, wet, asleep — is nothing at all: a hungry blacksmith is a blacksmith. Use the exact word — the clinical, technical or trade term — over the everyday one when it exists. No verb, no "qui", no second detail.
